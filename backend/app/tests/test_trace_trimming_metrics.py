...
def enable_v2_flag_and_reset_metrics():
...
@pytest.mark.asyncio
async def test_trace_over_20_events_is_truncated_and_metric_increments(seeded: AsyncClient) -> None:
    create_res = await seeded.post(
        "/api/sessions",
        json={"display_name": "trim-user", "condition": "adaptive", "mode": "gameplay_v2"},
    )
    ...
    trace = {
        ...
        "trace": [
            {"event_type": "hotspot_clicked", "hotspot_id": f"h_{i}", "elapsed_ms": i}
            for i in range(25)
        ],
    ...
    async with Session() as db:
        ...
        assert len(event.payload.get("trace", [])) <= 20

    assert metrics.get_counter("telemetry.trace.truncated") >= 1


@pytest.mark.asyncio
async def test_trace_event_over_10kb_increments_too_large_metric(seeded: AsyncClient) -> None:
    create_res = await seeded.post(
        "/api/sessions",
        json={"display_name": "large-trace", "condition": "adaptive", "mode": "gameplay_v2"},
    )
    assert create_res.status_code == 200
    session_id = create_res.json()["data"]["session_id"]
    ...
    trace = {
        ...
        "type": "interaction_trace",
        "trace": [
            {"event_type": "hotspot_clicked", "hotspot_id": huge, "elapsed_ms": 1}
        ],
        "response_time_ms": 10,
    }
